...
assert deny "branch move onto main" -- check-command "git branch -M old main"
assert deny "branch copy onto main" -- check-command "git branch -C old main"
assert deny "checkout bare main" -- check-command "git checkout main"
# Read-only `git branch` query forms must NOT be blocked just because they name main.
assert allow "branch --list main is read-only" -- check-command "git branch --list main"
assert allow "branch -a listing" -- check-command "git branch -a"
assert allow "branch --contains HEAD" -- check-command "git branch --contains HEAD"
assert allow "branch -vv verbose list" -- check-command "git branch -vv"
