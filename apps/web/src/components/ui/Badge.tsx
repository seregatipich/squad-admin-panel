 ...
 *
 * Текст берётся из токенов `*-ink` — ступень светлее самого токена состояния:
 * на собственной подложке тона токен состояния даёт около 4:1 и не проходит AA
 * для 11px (accent — 4.01, crit — 3.98), а `*-ink` даёт 4.96–6.55 на
 * bg-surface. Обоснование и значения — в globals.css. Измерено композитингом подложки
