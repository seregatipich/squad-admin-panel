...
interface BlameResp {
  lines: Array<{ text: string; version_id: string; author_player_id: string | null }>;
...
describe.skipIf(skip.skip)('config versioning: history + diff + blame + restore', () => {
  ...
  it('install created initial baseline: every file has ≥1 version row', async () => {
    ...
    expect(baselineRow?.author_player_id).toBeNull();
  ...
  it('GET /blame attributes v2 marker line to current user', async () => {
    ...
    if (marker?.author_player_id) expect(r.authors[marker?.author_player_id]).toBeTruthy();
  });
