...
describe('GET /api/v1/auth/steam/callback', () => {
  ...
  // VIPSUB-5 (#171) deliberately replaces the two assertions that used to pin
  // "no panel access ⇒ no session at all". Self-service requires a session for
  // a player whose role has no `panel_access`, so the callback now mints one
  // with scope `self_service` and sends them to `/me` instead of `/no-access`.
  // The panel stays unreachable for that session — enforced in
  // `plugins/auth.ts` and proved by `test/security/self-service-session.test.ts`.
  it('player without role gets a self-service session and lands on /me (sentinel already claimed)', async () => {
    ...
    const cookieHeader = (res.headers['set-cookie'] ?? '') as string | string[];
    const flat = Array.isArray(cookieHeader) ? cookieHeader.join('\n') : cookieHeader;
    expect(flat).toMatch(/__Host-sid=/);
  ...
  it('player whose role lacks panel access gets a self-service session, not a panel one', async () => {
    ...
    const rows = await h.db
      ...
      .from(sessions)
      .innerJoin(players, eq(players.id, sessions.playerId))
      .where(eq(players.steamId64, 76561198000000201n));
    expect(rows).toHaveLength(1);
    expect(rows[0]?.scope).toBe('self_service');
  });

  it('player whose role grants panel access still gets a panel-scoped session and lands on /', async () => {
    await h.db.update(panelMeta).set({ firstOwnerClaimed: true }).where(eq(panelMeta.id, 1));

    const roleId = uuidv7();
    await h.db.insert(roles).values({
      id: roleId,
      name: 'With Panel Access',
      panelAccess: true,
    });
    await h.db.insert(players).values({
      id: uuidv7(),
      steamId64: 76561198000000202n,
      canonicalName: 'RoleWithAccess',
      canonicalNameNormalized: 'rolewithaccess',
      roleId,
    });

    const NONCE = 'panelaccess-nonce';
    await h.redis.set(`steam-nonce:${NONCE}`, '{}', 'EX', 300);
    const u = new URLSearchParams({
      n: NONCE,
      'openid.ns': 'http://specs.openid.net/auth/2.0',
      ...
      'openid.return_to': `https://panel.test/api/v1/auth/steam/callback?n=${NONCE}`,
      'openid.response_nonce': '2026-04-25T12:00:00Zpanelaccess',
      'openid.signed': 'signed,op_endpoint',
      'openid.sig': 'sig',
    });
    const res = await h.app.inject({
    ...
    const rows = await h.db
      ...
      .from(sessions)
      .innerJoin(players, eq(players.id, sessions.playerId))
      .where(eq(players.steamId64, 76561198000000202n));
    expect(rows).toHaveLength(1);
    expect(rows[0]?.scope).toBe('panel');
  });
});
