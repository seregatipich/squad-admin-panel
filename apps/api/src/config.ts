...
const envSchema = z
  ...
  .superRefine((config, ctx) => {
    // Session and OpenID state cookies are `Secure` with the `__Host-` prefix,
    // and Steam binds the OpenID realm to this origin, so a production panel
    // must be served from a bare HTTPS origin.
    ...
    try {
      ...
      ctx.addIssue({
        ...
        message: 'PANEL_PUBLIC_URL must be an HTTPS origin in production',
      });
    }
  });
