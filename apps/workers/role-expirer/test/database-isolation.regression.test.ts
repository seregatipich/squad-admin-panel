...
describeIfDb('role-expirer package resource isolation', () => {
  it("runs against its worker slot's clone of the run's crash-recoverable template", () => {
    if (!DATABASE_URL) throw new Error('role-expirer test database was not provisioned');
    const templateUrl = inject('squadPackageTemplateUrl');
    if (!templateUrl) throw new Error('role-expirer package template was not provided');
    const template = new URL(templateUrl).pathname.slice(1);
    expect(template).toMatch(/^sqworker_[0-9a-f]{12}_role_expirer$/);
    expect(new URL(DATABASE_URL).pathname).toBe(`/${template}__w${process.env.VITEST_POOL_ID}`);
  });
});

describe('role-expirer package Redis isolation', () => {
