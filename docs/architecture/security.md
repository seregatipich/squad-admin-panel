...
| Untrusted/PR code executing on a persistent runner | Verification uses disposable GitHub-hosted VMs and runs only on trusted `dev`/`master` pushes or explicit dispatches. Production deployment remains self-hosted and has no pull-request trigger. `scripts/test-workflow-security.sh` rejects any workflow combining `pull_request`/`pull_request_target` with `runs-on: self-hosted`; `scripts/test-ci-runner-strategy.sh` additionally prevents CI from drifting back onto the deployment runner (#217, #286). |
