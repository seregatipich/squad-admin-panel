...
| Untrusted/PR code executing on a persistent runner | There is no persistent runner any more: verification (`ci`, on `master` pushes or explicit dispatches) and the tk104 stand deploy (`deploy-tk104`, on `dev` pushes) both run on disposable GitHub-hosted VMs, and neither has a pull-request trigger. The deploy reaches tk104 over SSH with a key whose `authorized_keys` entry is `restrict,command=` bound to a validator that accepts only a commit SHA and four image digests, and the key lives in the `tk104-dev` environment, which admits `dev` alone. `scripts/test-workflow-security.sh` rejects any workflow combining `pull_request`/`pull_request_target` with `runs-on: self-hosted`; `scripts/test-ci-runner-strategy.sh` rejects any self-hosted job (#217, #286). |
...
| No CSP / security headers on apps/web | apps/web/next.config.mjs's headers() sets Content-Security-Policy, X-Content-Type-Options: nosniff, and X-Frame-Options: DENY on every route (default-src 'self'). /servers/:id/configs adds only `font-src 'self' data:` (Monaco inlines its codicon icon font as a data: URI) and `worker-src 'self' blob:` (its language workers) — no third-party origin is allow-listed anywhere: the Monaco bundle is vendored into `apps/web/public/monaco/vs` at build time and served same-origin. `script-src` picks up `'unsafe-eval'` when `NODE_ENV` is not `production`, because `next dev` compiles client chunks with an eval-based devtool and the strict policy blocks the framework runtime outright; production builds emit no `eval` and keep the strict policy, asserted by [`apps/web/test/headers.test.ts`](../../apps/web/test/headers.test.ts). |
