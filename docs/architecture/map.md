...
| `auth.ts` / `me-tokens.ts` | `POST /auth/logout`, `GET /me`, `GET/DELETE /me/sessions[/:id]`, `/me/tokens` | inline 401 on `!req.user` | `revokeSession` + liveBus `session.revoked`; `clearCookie` |
