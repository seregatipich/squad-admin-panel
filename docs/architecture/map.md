...
| `apps/web` | Next.js 15 App Router, `output: 'standalone'`, rewrites `/api/*` → `API_URL` (`next.config.mjs:6-11`). **71** `page.tsx` routes, 16 settings groups under `(dashboard)/settings/`. Hand-rolled i18n, Russian-only (`src/i18n/config.ts:10`, `LOCALES = ['ru']`); UI copy is Russian |
...
| `player-dossier.ts` | `GET /players/:id/dossier` | inline `combatView` (own dossier exempt) | Eight independent drizzle queries stitched in JS; Redis cache 60 s with `x-cache: hit\|miss` |
...
| `whitelist.ts` / `whitelist-applications.ts` / `vip-tiers.ts` | `GET/PUT /whitelist/settings`, `POST /whitelist/members`, `POST /whitelist/import`, review queue | `whitelist:view\|edit`; `can_edit_roles` (**including the tier GET**) | Whitelist "membership" is just `players.role_id = panel_meta.whitelist_role_id`; every grant enqueues **outbox** rows + `invalidatePermissionCache` + `revokeAllForPlayer` |
...
| File | Role |
|---|---|
| `src/i18n/config.ts` | `LOCALES = ['ru']`, `DEFAULT_LOCALE = 'ru'`, `INTL_LOCALE` (BCP-47 tag for `Intl` formatting) |
| `src/i18n/dictionaries/ru.ts` | Russian dictionary — the only dictionary; exports `TranslationKey = keyof typeof ru` |
| `src/i18n/translate.ts` | `getDictionary`, `interpolate`, `createTranslator` |
| `src/i18n/LocaleProvider.tsx` | the app's **only** React context; `useTranslator()` (five production call sites: `TopNav`, `LogoutButton`, `connection-banner`, `ServerBar`, `login/page`) and `useIntlLocale()` (every `DateTime`/`formatAbsolute` call site) both still depend on it — `useLocale()` has none left after the DOSSIER-6 vehicle-name lookup dropped it, kept as a cheap, tested primitive alongside them |
| `src/i18n/errors.ts` | maps API error `code` → `errors.<code>` |

Dictionary keys are flat dot-namespaced strings — **73 keys**, across `app.*`, `nav.*` (45), `login.*`, `noAccess.*`, `connection.*`, `errors.*`. No nesting, no plural rules, no ICU, no date/number formatting. Interpolation is one regex over `{token}`, and a missing key returns the key itself rather than throwing (`translate.ts:45`). Completeness is enforced by the `Record<TranslationKey, string>` type at compile time; `src/i18n/i18n.test.ts` additionally asserts that no key resolves to an empty string.
...
| **Economy / leaderboards** (5) | `economy_settings`, `bonus_transactions`, `player_bonus_accruals`, `vip_tiers`, `expiry_notifications` | `economy_settings` holds the `k_online`/`k_boost`/`k_seed` multipliers and a `privilege_costs jsonb`. `bonus_transactions` is an append-only ledger. |
