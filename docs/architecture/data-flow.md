...
| External Squad server (`runtime='external'`) | TCP `{rcon_host}:{rcon_port}` and UDP A2S `{rcon_host}:{query_port}` (both `worker-rcon`); `SquadGame.log` over SSH `tail -F` when a `server_log_sources` row is enabled (`worker-log-ingest`) |
