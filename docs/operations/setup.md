...
**Reading its `SquadGame.log` too (combat, connects, matches).** Open **Сервер → Настройки → Источник логов (SSH)**, enter the game host, SSH user, and the absolute path to `SquadGame.log` (for a screen-hosted server usually `/opt/<install>/SquadGame/Saved/Logs/SquadGame.log`), and save: the panel generates its own key pair and shows the public key. Append that line to `~/.ssh/authorized_keys` of that user on the game host; within ~15 s worker-log-ingest connects, pins the host key, and the section shows «читается». The worker needs `APP_ENCRYPTION_KEY` (both compose files pass it). Chat lines are not in the log — they come only over RCON.
