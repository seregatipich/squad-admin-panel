...
без перезапуска API. После изменения задание проверяет внутри контейнера точные
SHA, режим и производный VIP-секрет, затем принимает внешний
...
выпуски не сбрасывают режим. Для аварийного отката сначала выключи продажи на
сайте, затем повтори задание с `vip_revision_mode=false`; один лишь откат image
панели не должен ослаблять сохранённую границу молча.
