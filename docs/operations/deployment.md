...
четыре `BSS_*`-ключа в `.env.tk104`, сохраняет права файла и не печатает
значение. Повторный выпуск безопасен и не накапливает временные файлы.
...
обычная операция выпуска. Все четыре задания `deploy-tk104` удаляют временный ключ
SSH с раннера в `always()`.
