...
```bash
REDIS_URL=redis://127.0.0.1:6379/14 pnpm --filter @squad/worker-automation test
```
...
Domain logic (rule evaluation, action dispatch) is not implemented — deferred to Phase 2. When implemented, tests must cover rule evaluation, action dispatch, and Redis consumer-group membership.
