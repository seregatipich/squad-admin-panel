...
- **`AdminReloadServerConfig` after every successful `Admins.cfg` write** (`src/rcon-reload.ts` → `requestAdminsCfgReload`). Closes SYNC-3 correction №1: Squad does **not** passively re-read `Admins.cfg`, so the panel must issue the RCON reload for permission changes to take effect without a container restart. The command is `XADD`'d onto worker-rcon's `rcon:commands:<server_id>` stream (`MAXLEN ~ 500`), byte-identical to the clan-guard / log-ingest / scheduler `sendRconCommand` helpers.
