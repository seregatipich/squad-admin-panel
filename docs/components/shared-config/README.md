...
- [`graceful-shutdown.ts`](../../../packages/shared-config/src/graceful-shutdown.ts) — единый контроллер завершения работников: регистрирует `SIGINT`/`SIGTERM` до первого асинхронного шага запуска, запоминает ранний сигнал и выполняет очистку ровно один раз.
