...
- Тени не используются **нигде, включая плавающие слои**: меню, палитра
  команд, диалог и всплывающее оповещение отделяются от фона материалом
  (`bg-surface/95 backdrop-blur-xl`) и рамкой, как `Menu` и `Modal`. Глубина
  передаётся поверхностью, как в тёмном оформлении Apple.
