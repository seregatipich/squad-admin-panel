...
- `/login` и `/setup` снова ведут на «Войти через Steam»; меню пользователя и шапка `/me` больше не ссылаются на bss.games, а глобальный выход завершает только сессии панели.
- Секция «Интеграция SquadJS» читает `GET /api/v1/servers/:id/rnsquadjs` и всегда показывает движок RNSquadJS.

## 2026-09-08 — Живой список игроков без таймера

### Changed

- «Игроки онлайн» больше не опрашивает API по таймеру. Список ведёт шина событий: worker-rcon публикует `rcon.roster` сразу после обновления состава (теперь каждые 5 секунд, а не 30), и строка появляется тогда, когда игрок зашёл. Единственные внеплановые перечитывания — возврат на вкладку и восстановление шины после обрыва, когда события были потеряны.

## 2026-09-08 — Ростер вверх страницы, живое обновление, история автовыбора карты

### Added

- На странице «Голосование за карту» появилась карточка «История изменений»: кто, когда и что менял, отпечаток версии и кнопка отката. Это та же история версий, что и у редактора конфигов. Если в версии есть слой, исчезнувший из каталога, экран называет его и предлагает «Откатить без них».

### Changed

- Блок «Игроки онлайн» поднят в самый верх страницы сервера — выше состояния и карты; выше него остаются только баннеры аварий.
- Метка «закрыт» у отряда заменена красным замком с подписью в подсказке и для скринридера: в колонке шириной в половину страницы название отряда важнее.
- Из шапки списка убраны подпись про обновление раз в 30 секунд и общий флажок «Выделить всех». Вместо подписи — индикатор возраста данных, как на карточке состояния; выделение осталось в шапке отряда. Запасной опрос ускорен с 30 до 10 секунд, основной путь — событие шины сразу после опроса сервера.

### Removed

- Страница «Сид-календарь» удалена вместе с вкладкой. API расписания сидинга, тик планировщика и предупреждения о пересечениях в календаре ротации не тронуты.

## 2026-09-07 — Игроки онлайн в две колонки по командам

### Changed

- Блок «Игроки онлайн» на странице сервера теперь выглядит как экран отрядов в игре: две колонки, по одной на команду, с названием фракции и счётчиками игроков и отрядов в шапке. Внутри колонки — командирский отряд первым, затем отряды по номеру, игроки без отряда в конце; в шапке отряда его номер, название, метка «закрыт» и заполненность `n/9`. Командир всегда идёт первым в своём отряде, у имени — звезда. Обе колонки есть всегда, даже пока одна из команд пуста; игроки без команды показываются отдельным блоком под колонками. Ниже 1280px колонки встают друг под другом.
- Колонки SteamID64 и EOS ID из таблицы убраны — в половине ширины им нет места; оба идентификатора остались в подсказке имени игрока и в досье. Рядом с именем показывается кит из роли (`USA_Rifleman_01` → «Rifleman»). Чекбокс «Выделить всех» переехал в шапку карточки.

## 2026-09-07 — Источник логов внешнего сервера

### Added

- В настройках внешнего сервера секция «Источник логов (SSH)»: хост, порт, пользователь, путь к `SquadGame.log`, переключатель чтения, состояние хвоста (читается / подключение / ошибка) и публичный ключ панели для `authorized_keys`; кнопки «Перевыпустить ключ» и «Удалить источник».

## 2026-09-05 — Подключение существующего сервера по RCON

### Added

- Мастер `/servers/new` получил переключатель «Установить на этом хосте / Подключить существующий». Второй режим — форма адреса, порта и пароля RCON плюс порты A2S и игры; отправляет `POST /api/v1/servers/external` и сразу открывает страницу сервера, без шага установки.
- В настройках внешнего сервера секция «RCON-подключение» (`PUT /external-connection`); пустое поле пароля оставляет сохранённый.

### Changed

- Внешний сервер помечен бейджем «внешний» в списке и в шапке раздела. У него нет кнопок «Старт/Стоп/Рестарт/Обновить игру», плиток CPU/RAM, лога контейнера и файлов логов; вместо них — адрес RCON и порт A2S. Подразделы «Конфиги», «Ротация», «Календарь ротации», «Мониторинг» скрыты, как и секции «Сеть», «Ресурсы», «Архив логов», «Лицензия» в настройках.
- Кнопка списка серверов переименована из «Установить новый» в «Добавить сервер» — способов теперь два.

## 2026-09-03 — Цель записи в «Последних действиях» больше не режется вслепую

### Fixed

- Дашборд сокращал `target_id` любой записи журнала до восьми символов, поэтому `host · localhost` показывался как `host · localhos`. Теперь до восьми символов сокращаются только UUID; остальные идентификаторы (`localhost`, `days:30`, `1`) выводятся целиком, длинные по-прежнему обрезаются CSS с полным значением в `title`.

## 2026-09-01 — Бесшовный переход между сайтом и панелью (#299)

### Changed

- `/login` проверяет существующий сеанс и, если его нет, один раз направляет пользователя в единый вход `bss.games`. После ошибки остаётся явная кнопка повтора без бесконечных переходов.
- Прямая кнопка и маршруты Steam удалены после боевой приёмки. В меню модератора и шапке самообслуживания появились ссылка на сайт, выход только из панели и глобальный выход из сайта и панели.
- Глобальный выход принимает адрес возврата только от API панели; клиент не доверяет адресу из запроса. Цели управления сеансом имеют высоту не менее 44 пикселей.

## 2026-08-30 — «Аккаунт»: статистика вперёд, во всю ширину, без выбора сервера

### Changed

- `/settings/account` больше не зажат в колонку `reading` (768px) раздела
  «Настройки». Маршрут вынесен в группу `(account)` —
  `app/(dashboard)/(account)/settings/account/` — адрес прежний, но каркас
  настроек его не оборачивает, и страница сама берёт `PageContainer
  width="full"`. Причина: на ней теперь таблицы оружия, техники и матчей,
  которым узкая колонка тесна; остальные два десятка экранов «Настроек»
  остаются на `reading`, как и решено в их layout.
- Порядок блоков: игровая статистика и последние матчи идут выше профиля.
  SteamID64 и число ключей — справка, которую смотрят раз в жизни.
- `DossierSection` принимает `serverFilter` (по умолчанию `true`). На своей
  странице он `false`: своя игровая статистика считается по всем серверам
  сразу, и выбор сервера там — лишний орган управления. Запрос за списком
  серверов в этом режиме не уходит вовсе. На карточке игрока выбор остаётся —
  админу он нужен.

## 2026-08-25 — Своя игровая статистика на странице «Аккаунт»

### Added

- Страница `/settings/account` показывает игроку его собственную внутриигровую
  статистику: блок «Игровая статистика» (те же вкладки «Скилл», «Оружие»,
  «Техника», «Киты», тренд K/D и «Уничтожено техники», что и на карточке
  игрока) и «Последние матчи». Оба блока читают маршруты по своему
  `player_id` из `GET /api/v1/me`; досье пропускает владельца сессии без
  `combat:view`, а сводка матчей и раньше была открыта любому, у кого есть
  доступ в панель.
- Плитка «Онлайн» в вкладке «Скилл» — время на серверах за выбранный период
  (`skill.online_seconds`), двенадцатая по счёту. Формат `Nч Nм`.

### Changed

- `DossierSection` и `RecentMatchesSection` со всей их обвязкой
  (`DossierSkillTab`, `DossierWeaponsTab`, `DossierVehiclesTab`,
  `DossierKitsTab`, `DossierSkillChart`, `dossier.ts`, `recent-matches.ts` и
  их тесты) переехали из `app/(dashboard)/all-players/[id]/` в
  `src/components/`. Файлы рядом с маршрутом принадлежат маршруту: второму
  потребителю пришлось бы импортировать их через `(dashboard)/all-players/[id]`,
  чего в этом приложении не делает никто.
- `DossierSection` принимает необязательный `title` (по умолчанию «Досье»):
  на своей же странице раздел называется «Игровая статистика», а не «досье».
- `formatKitTime` переименован в `formatPlayTime` — он форматирует не только
  время в ките, но и время на сервере.
- Пустые состояния вкладок досье перестали говорить об игроке в третьем лице
  («…этого игрока», «…с его участием»): на своей же странице это читалось как
  текст про кого-то другого. Чей это раздел, и так написано в заголовке.

## 2026-08-25 — Панель только на русском: убран переключатель языка

### Removed

- `LocaleSwitch.tsx` и `LocaleSwitch.test.tsx` — кнопки RU/EN, которые
  `TopNav.tsx` и `/login` показывали в правом углу. Панель никогда не
  предлагала переключить язык двум разным аудиториям одновременно, так что
  сам выбор был лишним.
- `apps/web/src/i18n/dictionaries/en.ts` и ключи `localeSwitch.*` в
  `ru.ts` — английский словарь и подписи переключателя не имели ни одного
  потребителя без `LocaleSwitch`. `ru.ts` остаётся единственным словарём и
  источником истины для ключей `t()`.
- `apps/web/src/i18n/server.ts` — читал cookie `locale` только для корневого
  layout; после того как `<html lang="ru">` стал константой, у него не
  осталось вызывающих.
- `apps/web/src/i18n/config.ts` схлопнут до одного локаля: `LOCALES = ['ru']`,
  `LOCALE_COOKIE`/`isLocale`/`resolveLocale` убраны как код без потребителей.
  `<html lang="ru">` больше не зависит от куки `locale` — старая cookie
  `locale=en` в браузере теперь просто ничего не значит. `INTL_LOCALE` и
  `useIntlLocale()` (добавленные накануне для форматирования дат) остаются:
  теперь это карта из одного значения, `ru` → `ru-RU`.
- `vehicleDisplayName()` в `all-players/[id]/dossier.ts` потерял параметр
  `locale` — досье игрока всегда показывает `name_ru`, с фолбэком на
  `name_en`, затем на `asset_id`; `DossierVehiclesTab.tsx` больше не читает
  `useLocale()`.

## 2026-08-24 — Даты в локали панели и честные состояния карточки хоста

### Fixed

- Даты печатались в локали браузера, а не панели: таблица игроков показывала
  `8/23/2026, 11:35:00 AM` в русском интерфейсе, а очередь жалоб в двух кликах
  — `22.08.2026, 03:33`. Виноваты 24 вызова `toLocaleString()` и его
  родственников без аргумента локали в 14 файлах; формат зависел ещё и от
  машины и версии ICU. Всё переведено на `DateTime` / `formatAbsolute` с тегом
  из нового `useIntlLocale()`.
- Карточка «Хост» на дашборде обещала метрики, которых не будет: пока
  `panel-host-bridge` лежит, она бесконечно крутила четыре скелетона
  «загружаем метрики» и подпись «Загружаем сведения о хосте…». Теперь
  недоступность агента отличается от ожидания данных и объясняется текстом.
- Карточка «Серверы» стояла на `h-full` и вытягивалась под соседнюю карточку
  хоста — три строки таблицы и триста пикселей пустоты под ними.

### Added

- `useIntlLocale()` (`src/i18n/LocaleProvider.tsx`) и карта `INTL_LOCALE`
  (`src/i18n/config.ts`): локаль интерфейса → тег BCP-47 для `Intl`
  (`ru` → `ru-RU`, `en` → `en-GB`, а не американский `en`).
- `formatClock()` — только время суток в том же 24-часовом формате, для
  колонок, где дата не несёт информации.
- `src/i18n/date-locale.regression.test.ts` — сканирует исходники и роняет
  сборку, если вызов форматирования без локали вернётся.
## 2026-08-24 — Страница «Аккаунт»: чистка и ник в шапке

### Fixed

- Колонка «Устройство» в таблице сессий показывала `Mozilla/5.0 (KHTML, like
  Gecko)` во всех строках сразу: жадная `.*` в регулярке разбора User-Agent
  доедала строку до последней скобки, а ею у любого браузера на Blink
  оказывается `(KHTML, like Gecko)`. Колонка существует ради того, чтобы
  отличать сессии друг от друга, и не отличала ни одной. Разбор переписан на
  пару «браузер + платформа» (`Opera 134 · Linux`, `Claude · Linux`).
- Число ключей доступа склоняется по-русски: было «53 ключей», стало
  «53 ключа». Форма берётся из `Intl.PluralRules`.

### Changed

- Из блока «Профиль» убраны строки «Идентификатор игрока» (внутренний UUID) и
  «Имя»: ник переехал в правый край шапки страницы, рядом с ним — история смен
  ников за кнопкой «ещё N ников» (модальное окно). Данные приходят из нового
  `GET /api/v1/me/names`.
- Со страницы убраны индикатор живости и секция «Выход». Выход остаётся в меню
  пользователя в шапке панели (`TopNav`), так что способ выйти не потерян.
- Разбор User-Agent, склонение числительных и выбор отображаемого ника вынесены
  в `settings/account/helpers.ts` по образцу соседних страниц раздела: из
  `page.tsx` они были недоступны тестам.


## 2026-08-22 — Редизайн: панель собрана из примитивов Apple HIG

### Added

- `docs/components/web/design-system.md` — контракт вёрстки: пять ступеней
  типографики от базовых 13px, 8-точечная сетка, четыре разрешённые ширины
  содержимого, три уровня поверхностей, цвет только как состояние, минимальные
  цели нажатия, обязательные состояния экрана, правила таблиц и сгруппированных
  списков.
- `apps/web/src/components/ui/` — 24 примитива, реализующих эти правила:
  каркас страницы (`PageContainer`, `PageHeader`), карточки, кнопки, таблицы,
  панель инструментов с поиском и пагинацией, состояния экрана (`Skeleton`,
  `EmptyState`, `InlineBanner`), модальные окна, поля и сгруппированные списки,
  сегментированные элементы, меню, бейджи, плитки показателей, единый формат
  времени и векторный набор значков. Примитивы не читают словарь переводов —
  весь человекочитаемый текст приходит пропсами.
- Экраны ожидания и ошибки для панели, публичной части и личного кабинета
  (`loading.tsx` / `error.tsx` в каждой группе маршрутов) и страница
  «страница не найдена». Раньше исключение в серверном компоненте отдавало
  служебный экран Next.js по-английски и без пути назад.
- Витрина раздела «Настройки» (`/settings`): двадцать страниц раздела
  существовали только пунктами выпадающего меню, и адрес `/settings` не
  открывался ничем.
- Каркас раздела «Сервер» (`servers/[id]/layout.tsx`): имя сервера как
  единственный `<h1>` раздела и сегментированная навигация по подстраницам
  вместо девяти текстовых ссылок со стрелками. Маршрут
  `/servers/[id]/combat-log` существовал, но ссылки на него не было нигде.
- Прямые действия над игроком в живом ростере сервера. Чтобы кикнуть одного
  человека, оператор проходил массовый сценарий из шести-семи шагов.

### Changed

- Все страницы панели переведены на примитивы: одна ширина вместо пяти,
  один заголовок страницы вместо девяти вариантов, липкие шапки таблиц с
  выравниванием чисел, единая панель фильтров, скелеты вместо строки
  «Загрузка…», пустые состояния, различающие «ничего нет» и «фильтр ничего не
  нашёл».
- Английские заголовки колонок в русских таблицах переведены (Total playtime,
  Last seen, Created, Actor, Hits, Slug, Permissions). Технические
  идентификаторы — SteamID64, EOS ID, RCON, CIDR, URL, SHA-256 — оставлены.
- Выпадающие меню верхней панели стали настоящими `role="menu"`: стрелки,
  Home/End, поиск по букве, Escape с возвратом фокуса. Командная палитра —
  combobox со списком `option` и `aria-activedescendant`.
- Всплывающие уведомления сведены в одну область: три из них прибивали себя к
  правому нижнему углу и накрывали друг друга.
- Полоса переключения серверов прилипает под верхней панелью; высота всей
  прилипающей хромы публикуется переменной `--chrome-h`, от которой
  отсчитываются шапки таблиц.

### Fixed

- Фокус-ловушки не было ни в одном модальном окне панели: `aria-modal`
  объявлялся, но фокус не удерживался, Escape работал в двенадцати файлах из
  тридцати восьми, возврат фокуса на триггер не делался нигде. Все окна
  переведены на нативный `<dialog>`.
- Окно массовой модерации уничтожало введённую причину кликом мимо панели;
  окно обновления закрывалось посреди работающей операции.
- 29 нативных `confirm()` заменены диалогами, которые называют, что именно
  произойдёт; необратимые операции над хостом требуют набрать точную строку.
- Вспомогательный текст `text-neutral-600` (#78787D) давал 3.87:1 на фоне
  страницы — ниже порога AA — и использовался 121 раз для смысловых состояний.
  Плейсхолдер красился в 2.5:1, хотя у 108 полей он был единственной подписью.
- Прилипающая шапка таблицы уезжала под полосу серверов на дашборде, серверах,
  статистике, матчах и в чате.
- `MetricsChart` растягивал SVG через `preserveAspectRatio="none"`, из-за чего
  форма кривой не соответствовала данным.
- Капслок снят со смысловых заголовков: 340 вхождений → 33 (остались служебные
  ярлыки над значениями). Кегль ниже 11px убран полностью (144 вхождения
  `text-[10px]` → 0).

## 2026-08-22 — Редизайн: палитра Apple HIG и верхняя навигация

### Changed

- Цветовая схема панели переведена на системные цвета Apple в тёмном
  исполнении. Поверхности — systemGray6/5/4 (`#1C1C1E` / `#2C2C2E` / `#3A3A3C`)
  вместо почти чёрного `#0A0A0A`: белый текст на `#1C1C1E` даёт 17:1 вместо
  21:1, что заметно легче для глаз за целую смену. Акцент — systemBlue
  повышенной контрастности `#409CFF` (4.9:1 на карточке; обычный `#0A84FF`
  давал 3.8:1 и не проходил AA для мелкого текста), состояния —
  systemGreen/Orange/Red в тех же вариантах.
- Токены в `apps/web/src/styles/globals.css` заданы двумя слоями:
  семантическим (`bg`/`surface`/`raised`, `ink*`, `accent`, `good`/`warn`/
  `crit`) для нового интерфейса и переопределением штатных шкал Tailwind
  (`neutral`/`sky`/`emerald`/`amber`/`red` плюс категориальные оттенки).
  В панели ~6200 цветовых классов на 79 страницах написаны через штатные
  шкалы — перенацеливание перекрашивает весь интерфейс сразу, не трогая
  разметку страниц. Оба слоя дают одинаковые цвета.
- Захардкоженные цвета графиков (recharts, рукописные SVG, палитра серверов
  в `lib/server-color.ts`) переведены на те же значения — CSS-переменные до
  них не доходят. Фирменные цвета Steam и Discord оставлены как есть.
- Шрифты Inter и JetBrains Mono подключены локально через `@fontsource`,
  без обращения к Google Fonts: панель должна одинаково выглядеть на хосте
  без интернета.

### Added

- `TopNav.tsx` — верхняя панель навигации высотой 46 px вместо бокового меню
  шириной 224 px. Пункты верхнего уровня: «Дашборд» (прямая ссылка),
  «Серверы», «Игроки», «Инструменты», «Сообщество», «Аудит», «Настройки».
  Разделы с несколькими колонками раскрываются мега-меню, привязанным к
  полосе, а не к кнопке. Меню открываются по клику и закрываются по Escape,
  клику вне полосы и при переходе. Содержит поле поиска (открывает палитру
  команд), меню пользователя, `LogoutButton` и `LocaleSwitch`.
- `ServerBar.tsx` — переключатель серверов под полосой: чип на сервер со
  статусом и текущим онлайном, только на `/dashboard`, `/servers`,
  `/statistics`, `/matches` и `/chat`. Данные из `GET /api/v1/servers`,
  обновление по событиям живой шины.
- В навигацию добавлены `/servers` и `/servers/new`, на которые боковое меню
  не ссылалось вовсе.
- `openCommandPalette()` в `lib/commandPalette.ts` — DOM-событие, которым
  верхняя панель открывает палитру: оба компонента клиентские и соседние под
  серверным layout, поэтому не могут делить состояние напрямую.

### Removed

- `SidebarNav.tsx` и ключ локализации `nav.brand` (верхняя панель бренд не
  рисует). Все проверки прав, гейт экономики, ключи локализации и живой
  счётчик жалоб перенесены в `TopNav` без потерь.

## 2026-08-11 — Понятный срок назначения роли (#268)

### Changed

- Обе формы назначения роли используют общий `RoleExpiryDateField`: вместо
  `datetime-local` с часами и зависимым от языка браузера порядком даты поле
  всегда показывает `ДД/ММ/ГГГГ` и открывает нативный календарь нажатием по
  всей видимой области.
- Выбранная дата хранится как конец того же UTC-дня
  (`23:59:59.999Z`), поэтому день включён полностью и стабильно возвращается в
  форму. Пустое значение по-прежнему означает бессрочную роль.
- Комментарий подписан как необязательная причина выдачи, видимая другим
  администраторам. Модальное окно `/users` и карточка игрока используют
  одинаковые текст и поведение.
