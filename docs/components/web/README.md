...
| `/settings/account` | `src/app/(dashboard)/(account)/settings/account/page.tsx` | Account identity (name + nickname history in the header), own in-game statistics («Игровая статистика» + «Последние матчи») and session management — list active sessions, revoke individual or all. |
