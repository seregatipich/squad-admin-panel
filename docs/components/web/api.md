...
| `/settings/account` | `(dashboard)/settings/account/page.tsx` | none | Account name (in-game, Steam persona as fallback) plus nickname history in the page header. Profile (SteamID64, permissions count). Own in-game statistics: the «Игровая статистика» block (the shared `DossierSection`, retitled) and «Последние матчи», both keyed on `player_id` from `GET /api/v1/me`. Active sessions table with individual and bulk revoke. Logout lives in the top-nav user menu, not on this page. Polls every 30 s. |
