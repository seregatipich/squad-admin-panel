...
On mount the page reads `GET /api/v1/me` once. When the selected file's `behavior` is `requires_restart` **and** the caller holds `server:restart`, a "Рестарт сервера" button appears; it `POST /api/v1/servers/:id/restart` behind a `confirm()` and is disabled while in flight. The RCON auto-reload no longer fires for these files (`reload.reason='not_hot_reload'`), so the restart is the operator's apply path.
...
## Автовыбор карты (`/servers/:id/map-vote`)

Экран управляет тем, какой слой панель поставит следующим. Он **ничего не пишет в `.cfg` игрового сервера** — ни в `LayerRotation.cfg`, ни в `Server.cfg`, ни в какой-либо `LayerVoting*.cfg`. Всё решение живёт в базе панели, а применяется по RCON командой `AdminSetNextLayer`, которую раз в матч отправляет тик планировщика (`apps/workers/scheduler/src/map-vote-tick.ts`). Поэтому экран работает и у внешнего сервера, у которого панель не держит файлов. Файловая ротация — это соседний раздел «Ротация» (`/servers/:id/rotation`), который действительно переписывает `LayerRotation.cfg`.

### Что делает каждый параметр

| Поле на экране | Где хранится | Что меняет |
|---|---|---|
| «Автовыбор карты» (переключатель) | `server_settings.map_vote_enabled` | Тик планировщика берёт сервер в работу только когда включено. Выключенный сервер ротация игры водит сама, панель не вмешивается. |
| «Правило выбора» | `server_settings.map_vote_selection` | `weighted_random` — жребий с весами кандидатов; `least_recently_played` — берётся слой, который дольше всех не играли. Правило исполняет `selectNextLayer` из `@squad/shared-config`, один и тот же код в предпросмотре и в тике. |
| «Кулдаун слоя» | `server_settings.map_vote_layer_cooldown` | Сколько последних матчей исключают повтор **того же слоя**. 0 — повтор разрешён сразу. |
| «Кулдаун карты» | `server_settings.map_vote_map_cooldown` | То же, но по **карте**: не даёт подряд выдать Yehorivka RAAS и Yehorivka AAS. |
| «Шаблон объявления» | `server_settings.map_vote_broadcast_template` | Текст, которым сервер объявляет выбранный слой. Пустое поле — объявления нет. |
| Строка кандидата: слой, вес, галка «включён» | `map_vote_candidates` | Пул, из которого идёт выбор. Вес влияет только на `weighted_random`. Выключенная строка остаётся в пуле, но в выборе не участвует — это способ временно убрать слой, не теряя вес. |

Слой, помеченный в каталоге как `deprecated`, добавляется только с явным подтверждением (409 `deprecated_layer_confirmation_required`), а в выборе исключается всегда — карточка «Предпросмотр» показывает причину исключения по каждому кандидату.

### Предпросмотр и история выборов

`GET /map-vote/preview` считает решение тем же `selectNextLayer` и с тем же зерном (id последнего матча), что и тик, поэтому «Будет выбран» — это ровно то, что применится, а не оценка. «История выборов» читает `map_vote_picks`: по строке на матч, с отметкой, дошла ли команда до сервера.

### История изменений — общая с редактором конфигов

Каждое сохранение (правила и пул) пишет версию в **ту же таблицу `config_versions`**, что и редактор конфигов, под именем файла `map-vote.json`: цепочка `parent_version_id`, автор, IP, сообщение и sha256 содержимого. Содержимое версии — канонический JSON состояния экрана (кандидаты отсортированы по имени слоя), поэтому повторное сохранение без изменений новую запись не создаёт.

- `GET /api/v1/servers/:id/map-vote/versions` — список версий (`can_restore` зависит от права `changemap`).
- `GET /api/v1/servers/:id/map-vote/versions/:vid` — содержимое и разобранный снимок.
- `POST /api/v1/servers/:id/map-vote/versions/:vid/restore` — откат: восстанавливает правила и пул, пишет аудит `server.map_vote.restore` и **сам становится новой версией**, так что история никогда не теряет шаг.

Имя `map-vote.json` намеренно не входит в `ALLOWED_CONFIG_FILES`: раздел «Конфиги» не должен его показывать, предлагать к редактированию или сверять с диском, на котором такого файла нет. Если версия ссылается на слой, исчезнувший из каталога, откат отвечает 409 `unknown_layers_in_version` со списком — экран показывает его и предлагает «Откатить без них».
