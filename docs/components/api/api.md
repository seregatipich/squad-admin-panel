...
| GET | `/api/v1/auth/steam/callback` | Временный путь отката переходного выпуска: проверяет Steam OpenID, создаёт сеанс и назначает первого Owner. В пользовательском интерфейсе вход через Steam не показывается; после приёмки единого входа маршрут удаляется. | none |
