...
| GET | `/api/v1/servers/:id/log-source` | Remote `SquadGame.log` source of an external server. `{ configured:false, status:null }` until set up; afterwards `{ configured:true, kind:'ssh', ssh_host, ssh_port, ssh_user, log_path, enabled, public_key, host_key_fingerprint, key_version, updated_at, status }` where `status` is worker-log-ingest's Redis key `log-source:status:<id>` (`state: connecting\|connected\|error`, `lines`, `last_line_at`, `error`) or `null`. 409 `not_external_server` for a panel-hosted row. | `server:view` |
| PUT | `/api/v1/servers/:id/log-source` | Create/update the SSH log source: `{ ssh_host, ssh_port?=22, ssh_user, log_path, enabled?=true, regenerate_key?=false }`. `log_path` must be absolute and match `[A-Za-z0-9._/-]` with no `..` (it is placed inside `tail -F -- '<path>'`). On the first PUT (or `regenerate_key`) the API generates an RSA-3072 key pair, stores the private key encrypted with `APP_ENCRYPTION_KEY` and returns the `authorized_keys` line as `public_key` (`key_version` increments). A changed host/port clears `host_key_fingerprint` so the worker re-pins the next host key (trust on first use). Audit `server.log_source.update`. | `server:edit_settings` |
| DELETE | `/api/v1/servers/:id/log-source` | Remove the source and its status key. 404 `log_source_not_configured` when none. Audit `server.log_source.delete`. | `server:edit_settings` |
| GET | `/api/v1/servers/:id/roster` | Live roster from worker-rcon's Redis snapshots. `players[]` come from `rcon:roster:{id}` (`ListPlayers`, resolved to panel `player_id` by EOS id, then SteamID64). `teams[]` (`{ team_id, name }`, the faction as `ListSquads` names it) and `squads[]` (`{ team_id, squad_id, name, size, locked, is_command_squad }`) come from `rcon:squads:{id}`, which expires 90 s after the last successful poll — both are `[]` whenever that key is missing, and the UI renders the roster from `players` alone in that case. The creator identity of a squad is not exposed; the roster rows carry `is_leader`. `{ polled_at: null, players: [], teams: [], squads: [] }` before the first poll. | `server:view` |
...
| GET | `/api/v1/servers/:id/sidecar` | Engine-neutral sidecar status. Returns `{ server_id, engine: 'squadjs2'\|'rnsquadjs', mode: 'production'\|'shadow'\|'legacy', cutover, status }`. `engine` is `SISMEMBER squadjs2:engine-servers`. `status` dual-reads the engine-neutral `sidecar:status:{id}[:shadow]` keys first and falls back to the legacy `rnsquadjs:status:*` ones for servers whose sidecar has not been replaced yet. Read-only, `audit:false`. | `server:view` |
| POST | `/api/v1/servers/:id/sidecar` | Switches engine and/or mode. Body `{ engine: 'squadjs2'\|'rnsquadjs', mode: 'production'\|'shadow' }`. Records the engine in `squadjs2:engine-servers`, deletes the abandoned engine's config dir (it holds the plaintext RCON password), stops **both** engines' containers, then launches the target one — so the `:shadow` stream never has two writers. `production` sequences exactly like the legacy cutover POST (SADD → 202 → one reconcile tick → re-check → launch); `shadow` starts the replacement before SREM. Audit `server.sidecar.switch`. | `server:stop` |
...
| GET | `/api/v1/players/:playerId/dossier` | DOSSIER-5 consolidated dossier: one payload with `skill` + `kd_trend`, both over the same seed-free matches (`matches.is_seed = false` — a warm-up round is not game statistics; outcome rules as in `/combat-summary`; `kd_trend` entries are `[{ month, kills, deaths }]` grouped from those very matches, so the chart cannot disagree with the summary), `weapons` (top `weaponsLimit`, default 20, max 100) + `weapons_total`, `vehicles` + `vehicle_kills` (LEFT-JOIN-enriched from `vehicle_catalog` with `name_en`/`name_ru`/`vehicle_class`; uncatalogued asset ids get `unlocalized: true` + null names), and `kits` from `player_kit_time`. `skill.online_seconds` is the time on the servers over the same window — the sum of the month rows' `online_seconds` without the `matches_played > 0` filter the trend applies, so a month spent seeding still counts. Query: `?from=&to=`, `serverId=<uuid\|all>` (default `all`), `weaponsLimit=`. Caveat: `serverId=<uuid>` filters only `kits` and `skill`/`kd_trend` — the weapon/vehicle aggregate tables carry no server dimension and always report lifetime totals (`period: "all"`). `damage`/`damage_dealt` serialize as `null`, never 0; a player without history → 200 with zeros/empty arrays. 404 `player_not_found` for an unknown id; cached 60 s (`x-cache: hit\|miss`). | `combat:view` (own dossier exempt) |
