...
- Единый вход покрыт [`auth-bss.test.ts`](../../../apps/api/test/auth-bss.test.ts), [`bss-sso.test.ts`](../../../apps/api/test/bss-sso.test.ts) и [`authenticated-player.test.ts`](../../../apps/api/test/authenticated-player.test.ts): точный callback и аудитория, PKCE, TTL и одноразовость, повторная проверка локального RBAC, разделение `panel`/`self_service`, ротация секрета, ограничения частоты, очистка прежней сессии и отсутствие кодов/SteamID64 в журналах. Интеграционный harness отдельно закрепляет, что `/api/v1/auth/bss/login` доступен, а удалённый `/api/v1/auth/steam/login` возвращает 404.
