...
6. **Independent client teardown** — `clientA.close()` does not affect `clientB`; `clientB.ping()` succeeds after `clientA` is closed.

## What is not covered by unit tests

- All 17 RPC method param shapes — covered by E2E forbidden/success pairs.
