...
## 9. Открытые вопросы к оператору

1. Какой digest `ghcr.io/breaking-squad/squadjs` пиновать стартово (последний
   verified или конкретный)? Нужен доступ пакета для репозитория панели.
2. Канарейка — тот же сервер, что в прошлой миграции?
3. Окна соака (24 ч по умолчанию) и размер батча (5) — подтвердить.
